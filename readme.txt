...
A. Medicine.java
   - The "Model" class.
   - Defines the attributes of a medicine (Name, Batch, Price, Qty, Date).
   - Handles date parsing and data encapsulation.

B. InventoryManager.java
   - The "Logic" class.
   - Contains the core algorithms for adding stock, calculating sale costs, 
     and scanning for expiry dates.
...
Step 2: Save all three files in the same folder.
Step 3: Open your terminal/command prompt in that folder.
Step 4: Compile the code using the command:
        javac MediStockApp.java
...
- Implementation of a Graphical User Interface (GUI) using JavaFX.
- Integration with SQL Database for larger storage capacity.
- Barcode scanning integration for faster billing.
